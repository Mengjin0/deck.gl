...
Operation used to aggregate all data point weights to calculate a pixel's color value. Valid values are `'SUM'`, `'MIN'` and `'MAX'`. `'SUM'` is used when an invalid value is provided.
